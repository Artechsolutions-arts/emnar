...
def _claim_next_pending_rag_upload(db_session: Session) -> str | None:
    ...
    """
    from onyx.db.custom_rag_models import AdminUpload, UserUpload
    
    # Check Admin first
    record_id = db_session.execute(
        select(AdminUpload.id)
        .where(AdminUpload.processing_status == "pending")
        .limit(1)
        .with_for_update(skip_locked=True)
    ).scalar_one_or_none()
    ...
    # Check User next
    record_id = db_session.execute(
        select(UserUpload.id)
        .where(UserUpload.processing_status == "pending")
        .limit(1)
        .with_for_update(skip_locked=True)
    ).scalar_one_or_none()
