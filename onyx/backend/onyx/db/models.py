...
from onyx.auth.schemas import UserRole, Department, UserStatusEnum, CompanyEnum
...
class User(SQLAlchemyBaseUserTableUUID, Base):
    ...
    department: Mapped[Department | None] = mapped_column(
        Enum(Department, native_enum=False), nullable=True
    )
    company: Mapped[CompanyEnum | None] = mapped_column(
        Enum(CompanyEnum, native_enum=False), nullable=True, default=CompanyEnum.VIRCHOW
    )
    is_deleted: Mapped[bool] = mapped_column(Boolean, default=False)
    status: Mapped[UserStatusEnum | None] = mapped_column(
        Enum(UserStatusEnum, native_enum=False), nullable=True, default=UserStatusEnum.ACTIVE
    )
...
class IndexAttempt(Base):
    ...
    an external webhook, or a single website crawl.
    """
...
class DocumentChunk(Base):
    ...
    department: Mapped[str | None] = mapped_column(String, nullable=True)
    doc_id: Mapped[str] = mapped_column(String, ForeignKey("document.id", ondelete="CASCADE"), nullable=False)
    year: Mapped[int | None] = mapped_column(Integer, nullable=True)
    data_type: Mapped[str | None] = mapped_column(String, nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    is_deleted: Mapped[bool] = mapped_column(Boolean, server_default=text("false"), default=False, nullable=False)
